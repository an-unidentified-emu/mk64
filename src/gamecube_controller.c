...
static const ContCmdData sContCmds[] = {
    { CONT_CMD_REQUEST_STATUS,     1,  3 },
    { CONT_CMD_READ_BUTTON,        1,  4 },
    { CONT_CMD_READ_MEMPAK,        3, 33 },
    { CONT_CMD_WRITE_MEMPAK,      35,  1 },
    { CONT_CMD_READ_EEPROM,        2,  8 },
    { CONT_CMD_WRITE_EEPROM,      10,  1 },
    { CONT_CMD_READ_RTC_STATUS,    1,  3 },
    { CONT_CMD_READ_RTC_BLOCK,     2,  9 },
    { CONT_CMD_WRITE_RTC_BLOCK,   10,  1 },
    { CONT_CMD_READ36_VOICE,       3, 37 },
    { CONT_CMD_WRITE20_VOICE,     23,  1 },
    { CONT_CMD_READ2_VOICE,        3,  3 },
    { CONT_CMD_WRITE4_VOICE,       7,  1 },
    { CONT_CMD_SWRITE_VOICE,       3,  1 },
    { CONT_CMD_KEY_PRESS_REQUEST,  2,  7 },
    { CONT_CMD_GCN_SHORT_POLL,     3,  8 },
    { CONT_CMD_GCN_READ_ORIGIN,    1, 10 },
    { CONT_CMD_GCN_CALIBRATE,      3, 10 },
    { CONT_CMD_GCN_LONG_POLL,      3, 10 },
    { CONT_CMD_RESET,              1,  3 }
};

#define NUM_COMMANDS (sizeof(sContCmds) / sizeof(sContCmds[0]))
unsigned int i;
const ContCmdData* findCmdData(enum ContCmds cmd) {
    for (i = 0; i < NUM_COMMANDS; i++) {
        if (sContCmds[i].cmd == cmd) {
            return (const ContCmdData*)&sContCmds[i].tx; // Cast to match the original struct
        }
    }
    return NULL;
}
...
void __osMakeRequestData(OSPifRamChCmd *readformat, enum ContCmds cmd) {
    const ContCmdData* cmdData;  // Declare at the top (C89-compliant)

    readformat->align = CONT_CMD_NOP;
    cmdData = findCmdData(cmd);  // Assign after declarations

    if (cmdData) {
        readformat->txsize = cmdData->tx;
        readformat->rxsize = cmdData->rx;
    } else {
        readformat->txsize = 0;
        readformat->rxsize = 0;  // Handle unknown command gracefully
    }
    readformat->cmd = cmd;
}
...
void __osContGetInitDataEx(u8* pattern, OSContStatus* data) {
    ...
    for (i = 0; i < __osMaxControllers; i++, ptr += sizeof(requestHeader), data++) {
        ...
        data->errnum = CHNL_ERR(requestHeader);
        if (data->errnum == 0) {
